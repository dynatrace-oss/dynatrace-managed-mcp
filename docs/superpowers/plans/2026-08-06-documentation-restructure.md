...
2. `## How it works` — the client launches the server as a child process and talks to it over stdio. Describe two distinct startup behaviours, which an earlier version of this plan wrongly conflated (verify both in the source before writing):
   - **Structural config validation**, which exits `1` on a malformed or empty config (`src/index.ts:117-129`). This runs before the mode branch, so it is not stdio-specific.
   - **The live-cluster check**, `validateManagedClients` (`:146`), which runs only in stdio. It does **not** exit the process. Environments that fail are dropped from the queryable set, and it **skips** any environment with no token (`if (!token) { … continue; }`), so an untokenized environment is neither checked nor dropped by it. A below-minimum-version cluster is likewise excluded rather than fatal — link `api-token.md` instead of restating that.

   Contrast in one line with remote mode, which has no live-cluster check at all, and link `setup-remote.md`.
...
**Heading style:** do not put numeric prefixes on the `##` headings. Digits survive the checker's `slugify`, so `## 2. Run the server` yields the anchor `2-run-the-server` rather than `run-the-server`. Task 5 dropped its numeric prefixes for this reason; match it so the two mode hubs read consistently. Keep the sequence obvious through ordering and prose instead.
...
2. `## How authentication works` — from legacy L214–237. The server holds **no** tokens. Every request carries the caller's per-environment tokens in one header:
   ...
   - There is no **live-cluster** check in HTTP mode. Structural config validation still runs and still exits `1` on a malformed config (`src/index.ts:117-129`, before the mode branch at `:138` — so it applies to both modes), but nothing contacts your cluster at startup: `validateManagedClients` is called only on the stdio branch (`:146`). A wrong URL or a bad token therefore surfaces per request as `401 Unauthorized: no valid Dynatrace token supplied` (`:325`), not at startup. Verify this reading yourself before writing it — an earlier version of this plan got the distinction wrong.
...
3. `## Common problems` — one `###` subsection per row below, each with symptom, cause and fix:
   ...
   | Exits 1 with `No valid environments found, stopping.`       | **Structural** config validation rejected every entry — a required field is missing or unparseable (`src/index.ts:124`, reached before the mode branch, so this happens in **both** modes). It does not mean the cluster was contacted. Check the required fields in [configuration fields](configuration.md#configuration-fields). Verify this against `validateEnvironments` before writing it; do not attribute this exit to the live-cluster check. |
