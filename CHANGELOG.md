...
- Added a [VS Code extension](docs/vscode-extension.md) in `extensions/vscode/`, published to the Visual Studio Marketplace as `dynatrace.dynatrace-managed-mcp`. It registers the server through `contributes.mcpServerDefinitionProviders`, so it appears in the VS Code MCP server list without hand-written configuration. The extension **bundles** the server and runs it on the Node.js runtime that ships with VS Code, so it requires neither Node.js on `PATH` nor access to the npm registry - the case that matters for air-gapped Managed deployments. API tokens are stored in VS Code SecretStorage (the OS keychain) rather than `settings.json` or a `.env` file, and are injected only in `resolveMcpServerDefinition`, immediately before the server process starts. A `dynatraceManagedMcp.runtime` setting switches back to `npx` for users who would rather track npm releases than extension releases
- Every tagged release now attaches the packaged `.vsix` to its GitHub release, so the extension can be installed with `code --install-extension` on machines that cannot reach the Marketplace
- `npm run version:check` now validates the VS Code listing icon by reading the dimensions out of the PNG header. The Marketplace requires at least 128x128 but `vsce package` accepts a smaller icon silently, so the rejection would otherwise happen mid-release while the release job is already uploading

### Fixes

- Fixed `list_events` printing `NaN` in its "Next Steps" section
- Corrected the `apiEndpointUrl` example in `.cursor-plugin/plugin.json` and the `dynatrace-managed` skill. It showed a `https://dynatrace.example.com/e/<env-id>/api` value, but the server appends `/e/<environmentId>` itself, so pasting that example produced a doubled path and every request failed. `apiEndpointUrl` is the cluster base URL, as the table in [README.md](README.md) already documented
...
- Widened `engines.node` from `>=26.5.1 <27` to `>=24`. The VS Code extension host ships its own Node.js runtime, currently 24.x, and the bundled extension runs the server on it - the old pin contradicted that. The previous range was a policy pin tracking `.nvmrc` and the Docker base image, both of which stay on 26.5.1; the server itself uses no API newer than Node 24
- `npm run version:check` now covers `extensions/vscode/package.json` as a seventh version-bearing manifest, and additionally asserts that it declares a `publisher` and `engines.vscode`, that `main` matches the path `esbuild.mjs` writes, that a `contributes.mcpServerDefinitionProviders` entry exists whose `id` is referenced by `src/extension.ts`, and that the `README.md` and `LICENSE` vsce packages into the `.vsix` are present. An extension manifest with no provider entry packages and installs cleanly and then contributes no MCP server at all, which is the same silent failure the Claude Code checks guard against
- `main.yml` now declares `permissions: {}` at the workflow level and grants each job only `contents: read`, so the `GITHUB_TOKEN` no longer inherits the repository default. `release.yml` already did this
